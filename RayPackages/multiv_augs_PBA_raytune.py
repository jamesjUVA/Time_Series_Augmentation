...
def window_slice(x, mag=.5):
    ...
    ret = np.zeros_like(x)
    for dim in range(x.shape[1]):
        ret[:,dim] = np.interp(np.linspace(0, target_len, num=x.shape[0]), np.arange(target_len), x[starts[0]:ends[0],dim]).T
    return ret
...
def window_warp(x, mag=.5):
    ...
    for dim in range(x.shape[1]):
        start_seg = x[:window_start,dim]
        window_seg = np.interp(np.linspace(0, warp_size-1, num=int(warp_size*warp_scale)), window_steps, x[window_start:window_end,dim])
        end_seg = x[window_end:,dim]
        warped = np.concatenate((start_seg, window_seg, end_seg))
        ret[:,dim] = np.interp(np.arange(x.shape[0]), np.linspace(0, x.shape[0]-1, num=warped.size), warped).T
    return ret

def axes_rotation(x,mag=0):
    ## Constant for PBA
